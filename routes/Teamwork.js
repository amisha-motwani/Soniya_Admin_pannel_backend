...
const TeamworkSchema = require("../models/Teamwork");
...
//   try {
//     const notes = await TeamworkSchema.find();
//     res.json(notes);
//   } catch (error) {
//     console.error(error);
...
//   }
// });
router.get("/fetchAll/Teamwear", getMiddleware, async (req, res) => {
  try {
    // Find all documents, sort by _id in descending order (latest first)
    const notes = await TeamworkSchema.find().sort({ _id: -1 });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Create a new TeamworkSchema instance with the uploaded file path
      const note = new TeamworkSchema({
        title,
        description,
        price,
        color,
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      note = await TeamworkSchema.findByIdAndDelete(req.params.id);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const notes = await TeamworkSchema.find(filter).sort(sortOption);
